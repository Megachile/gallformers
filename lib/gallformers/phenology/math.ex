defmodule Gallformers.Phenology.Math do
  ...
  Season index integrates positive, latitude-adjusted daylight over a fixed
  365-day year. It is retained for the explorer's optional selection lens and
  matching CSV filter, not for event predictions. The client counterpart is
  `assets/js/hooks/season_index.js`.

  The empirical correction in `eq/2` is inherited from the original R model;
  this is not literal accumulated daylight or a validated physiological clock.
  Its Southern Hemisphere behavior has not been calibrated. The event model
  uses `Gallformers.Phenology.SeasonalClock` instead.
  """
  ...
  any non-positive target. This inverse belongs to the legacy selection lens,
  not to the seasonal-landmark event predictor.
  """
